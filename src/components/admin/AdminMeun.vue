<template>
	<div class="side-menu-wrapper">
		...
		>
        	...
        	<el-menu-item
					index="10"
					:route="{ name: 'File', query: { fileType: 10, filePath: '/' } }"
					class="my-user"
				>
					<i class="el-icon-share"></i>
		...
		<template v-else>
			...
			>
				<el-submenu index="myFile" class="my-file">
					...
					<el-menu-item
						index="0"
						:route="{ name: 'Admin', query: { fileType: 0 } }"
					>
						<i class="el-icon-files"></i>
						<span slot="title">系统文件</span>
					</el-menu-item>
					<el-menu-item
						index="1"
						:route="{ name: 'Admin', query: { fileType: 1 } }"
					>
						<i class="el-icon-notebook-1"></i>
						<span slot="title">用户文件</span>
					</el-menu-item>
          <el-menu-item
						index="2"
						:route="{ name: 'Admin', query: { fileType: 2 } }"
					>
				...
				<el-menu-item
					index="3"
					:route="{ name: 'Admin', query: { fileType: 3 } }"
					class="recovery"
				>
				...
				<el-menu-item
					index="4"
					:route="{ name: 'Admin', query: { fileType: 4 } }"
					class="user-file"
				>
        	...
        	<el-menu-item
					index="5"
					:route="{ name: 'Admin', query: { fileType: 5} }"
					class="my-router"
				>
        ...
        <el-menu-item
					index="6"
					:route="{ name: 'Admin', query: { fileType: 6} }"
					class="my-logs"
				>
        	...
        	<el-menu-item
					index="7"
					:route="{ name: 'Admin', query: { fileType: 7} }"
					class="my-system"
				>
...
export default {
	...
	watch: {
		...
		isCollapse(newValue) {
			localStorage.setItem('left_meun', newValue)
	...
	created() {
		this.isCollapse = localStorage.getItem('left_meun') === 'true' //  读取保存的状态
	},
	mounted() {
