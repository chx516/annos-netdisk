...
axios.defaults.baseURL = "http://localhost:8080/mydom"
// POST 请求头
axios.defaults.headers.post['Content-Type'] =
  'application/x-www-form-urlencoded'
// axios.defaults.headers['Token'] = getToken()
// axios.defaults.headers.get['Token'] = getToken()
...
axios.interceptors.request.use(
  (config) => {
    config.headers['token'] = common.getCookies(globalConfig.tokenKeyName) === undefined ? false : common.getCookies(globalConfig.tokenKeyName)
    return config
  },
  (error) => {
    console.log(error)
