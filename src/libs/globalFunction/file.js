...
const fileFunction = {
  ...
  getDownloadFilePath(row) {
    const path = `${config.baseContext}/filetransfer/downloadfile?userFileId=${row.userFileId
  ...
  getFileOnlineViewPathByOffice(row) {
    ...
    fileUrl = `${location.protocol}//${location.host}${config.baseContext
      }/filetransfer/preview?userFileId=${row.userFileId
